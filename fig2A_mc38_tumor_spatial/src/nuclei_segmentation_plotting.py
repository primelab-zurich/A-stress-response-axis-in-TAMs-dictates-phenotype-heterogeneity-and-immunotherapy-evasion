...
def plot_mask_and_save_image(title, gdf, img, cmap, output_name=None, bbox=None):
    ...
    axes[0].imshow(cropped_img, cmap='gray', origin='upper')
...
def plot_gene_and_save_image(title, gdf, gene, img, adata, bbox=None, output_name=None):
    ...
    # Plot the filtered polygons on the second axis
    filtered_gdf.plot(column=gene, cmap='inferno', legend=True, ax=ax)
    ax.invert_yaxis()
    ax.set_title(title)
    ax.axis('off')
...
def plot_obs_key_and_save_image(title, gdf, obs_key, img, adata, bbox=None, output_name=None, **kwargs):
    ...
    # Find a gene of interest and merge with the geodataframe
    obs_values = sc.get.obs_df(adata, obs_key).rename_axis('id').reset_index()
    merged_gdf = gdf.merge(obs_values, left_on='id', right_on='id', how = 'right')
    ...
    # Plot the filtered polygons on the second axis
    filtered_gdf.plot(column=obs_key, cmap='Greens', legend=True, ax=ax, **kwargs)
    ax.invert_yaxis()
    ax.set_title(title)
    ax.axis('off')
    ax.legend(loc='upper left', bbox_to_anchor=(1.05, 1))
...
def plot_clusters_and_save_image(title, gdf, img, adata, bbox=None, color_by_obs=None, output_name=None, color_list=None):
    ...
    # Plot the filtered polygons on the second axis
    plot = filtered_gdf.plot(column=color_by_obs, cmap=custom_cmap, ax=ax, legend=True)
    ax.invert_yaxis()
    ax.set_title(title)
